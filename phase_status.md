...
- Bernini-A14B is available as a WAN 2.2 model mode with automatic Bernini task prompt selection for text-only (`t2v`), image/keyframe (`i2v`), source-video (`v2v`), Director character-reference (`r2v`), and source-plus-reference (`rv2v`) timelines. Bernini Core runtime uses ComfyUI Core `BerniniConditioning`: timeline image/video media is treated as source/background `source_video` context, while prompt-tagged Director character references are passed as subject `reference_images`. `ads2v` and other non-character-reference Bernini tasks remain deferred.

Phase 12 completes the original roadmap hardening pass. Phase 13 through Phase 16 plus Bernini-A14B are post-roadmap WAN implementation steps. WAN 2.2 now has Config, Planner, and dual-model Runtime nodes with supported ComfyUI Core execution paths for standard WAN and Bernini conditioning, but Timed visual keyframes are planned/debug-visible only; WanVideoWrapper, S2V/audio conditioning, Animate, vanilla WAN subject reference images, and non-character-reference Bernini tasks remain deferred. The current LTX path covers text, image, video, provided audio, native-audio gating, identity/reference helpers, privacy mode, prompt optimizer, and clear failure scenarios.
...
Next WAN work should focus on Timed keyframe execution, WanVideoWrapper adaptation, source-video/audio-conditioned WAN modes, Animate/S2V, or broader non-character Bernini reference tasks depending on which workflow is chosen.
